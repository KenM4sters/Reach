...
void main() 
{
    vec3 ambient_shading = material.Ambient * light.AmbientColor;
    // Diffuse
    ...
    float specular_intensity = pow(max(dot(view_dir, crit_reflection), 0.0), material.Shininess);
    vec3 specular_shading = material.Specular * specular_intensity;
