...
std::shared_ptr<Framebuffer> Framebuffer::Create(FramebufferConfig& config, FramebufferType type, bool drawQuad) 
{
    switch(Renderer::m_rendererAPI->GetAPI()) 
    {
        case API::VOID:
            ...
            break;
        case API::OPEN_GL:
            return std::make_shared<OpenGLFramebuffer>(config, type, drawQuad);
            break;
        case API::VULKAN:
